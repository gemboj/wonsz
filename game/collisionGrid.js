WONSZ.CollisionGrid = function(input) {
...
WONSZ.CollisionGrid.prototype.convertAABBPlane = function(x, plane) {
...
//converts point from world space to collisionGrid space
WONSZ.CollisionGrid.prototype.convertPoint = function(x) {
    var gridCount = [];
    for (var i = 0; i < 3; i++) {
...
WONSZ.CollisionGrid.prototype.insertObject = function(Obj) {
    var tempTab;
    if (typeof Obj.special == "undefined") {
        tempTab = this["check" + Obj.getBoundingVolume().getName() + "Grid"](Obj.getBoundingVolume());
    }
    else {
...
WONSZ.CollisionGrid.prototype.updateObject = function(Obj) {
    this.deleteObject(Obj);
    this.insertObject(Obj);
}

WONSZ.CollisionGrid.prototype.deleteObject = function(Obj) {
...
WONSZ.CollisionGrid.prototype.checkAABBGrid = function(AABB) {
    var tempTab = [];
...
WONSZ.CollisionGrid.prototype.checkAAPlaneGrid = function(AAPlane){
    var tempTab = [];
...
WONSZ.CollisionGrid.prototype.checkAABBagainstAABB = function(AABB1, AABB2) {
...
WONSZ.CollisionGrid.prototype.checkAABBagainstAAPlane = function(AABB, AAPlane){
    for(var i = 0; i < 3; i++){
...
WONSZ.CollisionGrid.prototype.checkBoundingVolumeCollision = function(BV) {
    ...
    for (var i = 0; i < tempTab.length; i++) {
        ...
        for (var j = 0; j < tempTab2.length; j++) {
            if (typeof tempTab2[j].special == "undefined") {
                var objBV = tempTab2[j].getBoundingVolume();
                var name = tempTab2[j].getBoundingVolume().getName();
            }
            else{
...
WONSZ.CollisionGrid.prototype.sqDistPointSegment = function(a, b, c) {
...
WONSZ.CollisionGrid.prototype.clsPntOnSeg = function(a, b, c) {
    var ab = [b[0] - a[0], b[1] - a[1], b[2] - a[2]],
        ac = [c[0] - a[0], c[1] - a[1], c[2] - a[2]];
 ...
 */
WONSZ.CollisionGrid.prototype.vecPointPlane = function(Q, P, n) {
    var pq = [Q[0] - P[0], Q[1] - P[1], Q[2] - P[2]];
    var temp = vec3.dot(n, pq);
    return [-n[0] * temp, -n[1] * temp, -n[2] * temp];
};

WONSZ.CollisionGrid.prototype.initGrid = function() {
    var tempGrid = [];
